...
app.post('/api/charge', async (req, res) => {
    ...
    console.log(`⚡ [Headless Browser] Target: ${cleanSite} | Buyer: ${profile.email}`);
    ...
    try {
        ...
        let gatewayResponsePayload = "";
        page.on('response', async (response) => {
            const url = response.url();
            if (url.includes('/checkout') || url.includes('/pay') || url.includes('payment') || url.includes('sessions')) {
                try {
                    const text = await response.text();
                    if (text && (text.includes('insufficient') || text.includes('declined') || text.includes('three_d_secure') || text.includes('thank_you') || text.includes('error'))) {
                        gatewayResponsePayload = text;
                    }
                } catch (e) {}
            }
        });
        ...
        // Safely trigger the real submit button inside the browser DOM context
        ...
        for (let i = 0; i < 16; i++) {
            if (gatewayResponsePayload) break;
        ...
        const combinedResultText = (pageText + " " + gatewayResponsePayload).toLowerCase();

        // Strict True-Success Check
        const isTrueCharged = 
            (finalUrl.includes('thank_you') || combinedResultText.includes('thank_you')) && 
            (combinedResultText.includes('order_number') || combinedResultText.includes('transactions')) &&
            !combinedResultText.includes('error') &&
            !combinedResultText.includes('declined') &&
            !combinedResultText.includes('insufficient');
        ...
        if (isTrueCharged) {
        ...
        } else if (combinedResultText.includes('insufficient_funds') || combinedResultText.includes('insufficient funds')) {
        ...
        } else if (combinedResultText.includes('three_d_secure') || combinedResultText.includes('challenge') || combinedResultText.includes('3d_secure')) {
        ...
        } else if (combinedResultText.includes('incorrect_cvc') || combinedResultText.includes('security code')) {
            ...
            let declineReason = "DECLINED_BY_PROCESSOR";
            if (combinedResultText.includes('incorrect_number') || combinedResultText.includes('incorrect number')) declineReason = "INCORRECT_CARD_NUMBER";
            else if (combinedResultText.includes('expired')) declineReason = "EXPIRED_CARD";
            else if (combinedResultText.includes('stolen')) declineReason = "STOLEN_CARD";

            return res.json({ status: "Dead", message: declineReason, gateway: "Shopify Payments", price: formattedPrice, site: cleanSite });
        }
...
app.listen(PORT, () => console.log(`Headless Browser Engine running on port ${PORT}`));