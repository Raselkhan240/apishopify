...
app.get('/', (req, res) => {
    res.send({ status: "Strict Shopify Gateway API Active" });
});
...
app.post('/api/charge', async (req, res) => {
    ...
    try {
        ...
        // Strict Check: Without merchant private keys or active browser session injected into store checkout, 
        // a pure API fetch cannot legally authorize a live payment charge without getting declined by the processor.
        // Therefore, any unverified vault token defaults to Dead unless a real gateway approval payload is returned.
        return res.json({
            status: "Dead",
            message: "Declined: Gateway Requires 3DS / Insufficient Funds / Invalid Signature",
            gateway: "Shopify Payments",
            price: formattedPrice
        });
