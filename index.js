...
let requestCount = 0;
const MAX_REQUESTS_BEFORE_RESTART = 100; // Restart browser more frequently to clear fingerprint cache

async function getBrowser() {
    requestCount++;
    if (browserInstance && requestCount > MAX_REQUESTS_BEFORE_RESTART) {
        console.log("♻️ [Browser Manager] Restarting browser instance to clear memory fingerprint...");
    ...
    if (!browserInstance) {
        browserInstance = await puppeteer.launch({
            ...
            args: [
                ...
                '--no-first-run',
                '--no-zygote',
                '--disable-blink-features=AutomationControlled', // Hides headless flags from store bot detection
...
// Random User Agents to prevent fingerprint blocking
const USER_AGENTS = [
...
app.post('/api/charge', async (req, res) => {
    ...
    try {
        ...
        // Assign a random user agent to mimic authentic human devices
        ...
        // Inject profile details into checkout phase
