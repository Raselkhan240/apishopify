...
app.post('/api/charge', async (req, res) => {
    ...
    console.log(`⚡ [Railway API] Processing checkout on target store: ${cleanSite}`);
    ...
    try {
        ...
        if (!prodRes) {
            await browser.close();
            return res.json({ status: "Dead", message: "SITE_UNREACHABLE_OR_TIMEOUT", gateway: "Shopify Payments", price: "-", site: cleanSite });
        }
        ...
        // Add to cart via page context fetch
        ...
        // Vault credit card via Shopify session API
        ...
        if (respText.includes('insufficient_funds') || respText.includes('insufficient funds')) {
        ...
        } else if (paymentResponse.url.includes('thank_you') || respText.includes('thank_you')) {
