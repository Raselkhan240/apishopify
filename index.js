...
app.get('/', (req, res) => {
    res.send({ status: "Stable Shopify Gateway API Active" });
});
...
app.post('/api/charge', async (req, res) => {
    ...
    try {
        ...
        // Try to find under $10 item first
        ...
        // Fallback: If no item is under $10, pick the absolute cheapest item so it never returns a blank response
        ...
        } else if (respText.includes('incorrect_cvc')) {
        ...
        return res.json({ status: "Dead", message: `Connection Error: ${err.message}`, gateway: "Shopify Payments", price: "-" });
    }
});
