...
app.get('/', (req, res) => {
    res.send({ status: "Strict Under-$10 Shopify API Active" });
});
...
app.post('/api/charge', async (req, res) => {
    ...
    try {
        ...
        // 1. Fetch store products (up to 50 items to find one under $10)
        const prodRes = await fetch(`${cleanSite}/products.json?limit=50`, fetchOptions);
        if (!prodRes.ok) {
        ...
        // Search for a product variant strictly priced between $0.01 and $10.00
        ...
        // STRICT CHECK: If no product is under $10, reject immediately instead of picking an expensive item
        if (!selectedVariantId) {
            return res.json({ 
                status: "Dead", 
                message: "Skipped: No products found under $10 on this store", 
        ...
        // 2. Add the under-$10 item to cart session via proxy
        const cartOptions = {
            method: 'POST',
            headers: { 
